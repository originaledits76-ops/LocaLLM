...
export const ModelCatalog: React.FC<ModelCatalogProps> = ({
  ...
  // Ensure we have recommendations for all available models
  const modelRecList = recommendations.length > 0
    ? recommendations
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-zinc-100">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-medium mt-0.5">
              100% private local inference stored in IndexedDB with WebGPU acceleration
            </p>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-zinc-50 border border-zinc-200/80 flex items-start gap-3">
          ...
          <div className="space-y-1 text-xs">
            ...
            <p className="text-zinc-600 leading-relaxed">
              Model weight shards and WASM binaries are stored directly in your browser's persistent IndexedDB database (<code>tvmjs</code> store), eliminating cache eviction risks and ensuring offline reliability.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {modelRecList.map((rec) => {
          ...
          return (
            ...
            >
              {/* Top Bar: Recommendation & Active Status */}
              <div className="flex flex-wrap items-center justify-between gap-2 pb-1">
                <div className="flex items-center gap-2">
                  {rec.isBestPick && (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-[#18181b] text-[#c7f43a] shadow-xs">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-zinc-100 text-zinc-700">
                    {model.quantization}
                  </span>
                </div>
                ...
                {rec.reason && (
                  <div className="mt-2.5 p-2.5 rounded-xl bg-zinc-50 border border-zinc-100 text-[11px] text-zinc-600 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                ...
                <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-100">
                  <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider block">Memory Req</span>
                  <span className="font-bold text-zinc-900 mt-0.5 block">{model.minRamGB}GB min / {model.recommendedRamGB}GB rec</span>
                </div>
                <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-100">
                ...
                <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-100">
                  <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider block">Storage</span>
                  <span className="font-bold text-zinc-900 mt-0.5 block">IndexedDB</span>
                </div>
              </div>
